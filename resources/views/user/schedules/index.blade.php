...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="bg-white rounded-lg shadow p-4 mb-8">
        <form method="GET" action="{{ route('user.schedules.index') }}" class="flex flex-wrap items-end gap-4">
            <div>
                <label for="consultant" class="block text-sm font-medium text-gray-700 mb-1">コンサルタント</label>
                <select name="consultant" id="consultant" class="border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
            ...
            <div>
                ...
                <a href="{{ route('user.schedules.index') }}" class="ml-2 inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-md text-sm text-gray-700 hover:bg-gray-50 transition">
                    リセット
                </a>
            </div>
        </form>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($schedules as $schedule)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                <a href="{{ route('user.consultants.show', $schedule->consultant) }}" class="text-indigo-600 hover:text-indigo-800 hover:underline">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right">
                                <a href="{{ route('user.bookings.create', $schedule) }}"
