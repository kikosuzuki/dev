...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <h1 class="text-2xl font-bold text-gray-900 mb-6">お気に入りコンサルタント</h1>
    ...
    @if($favorites->isEmpty())
        ...
        <div class="bg-white rounded-lg shadow p-12 text-center">
            ...
            <p class="text-gray-500 text-lg">お気に入りのコンサルタントはまだありません。</p>
            <a href="{{ route('user.consultants.index') }}" class="inline-flex items-center mt-4 px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded-md hover:bg-indigo-700 transition">
                コンサルタントを探す
            </a>
        </div>
    @else
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach($favorites as $consultant)
                <div class="bg-white rounded-lg shadow hover:shadow-lg transition-shadow duration-200 overflow-hidden" x-data="{ removed: false }" x-show="!removed" x-transition>
                    <div class="p-6">
                        {{-- Photo / Avatar --}}
                                ...
                                <div class="flex items-center">
                                    @for($i = 1; $i <= 5; $i++)
                                        ...
                                        @elseif($i - 0.5 <= $consultant->consultantProfile->average_rating)
                                            <svg class="w-5 h-5 text-yellow-400" fill="currentColor" viewBox="0 0 20 20">
                                                <defs>
                                                    <linearGradient id="half-star-fav-{{ $consultant->id }}-{{ $i }}">
                                                        <stop offset="50%" stop-color="currentColor"/>
                                                        <stop offset="50%" stop-color="#D1D5DB"/>
                                                    </linearGradient>
                                                </defs>
                                                <path fill="url(#half-star-fav-{{ $consultant->id }}-{{ $i }})" d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                                            </svg>
                                        @else
