            ...
            <div class="p-6">
                ...
                <div class="flex items-center space-x-6">
                    @if($profile && $profile->photo)
                        <div class="shrink-0">
                            <img class="h-20 w-20 object-cover rounded-full" src="{{ Storage::url($profile->photo) }}" alt="プロフィール写真">
                        </div>
                    @else
                ...
                @if($profile && $profile->photo)
                    <div class="mt-3">
                        <form action="{{ route('consultant.profile.photo.delete') }}" method="POST"
                              onsubmit="return confirm('プロフィール写真を削除しますか？')">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="inline-flex items-center px-3 py-1.5 text-xs font-medium text-red-700 bg-red-50 border border-red-200 rounded-md hover:bg-red-100 transition">
